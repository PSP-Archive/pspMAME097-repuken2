...
static char currentAppDirectory[1024];
...
static char *convert_path(char *buf, const char *path)
{
	...
	strcpy(dirActual,getCurDir());
    dirActualLen =strlen(dirActual);

	if (strchr(path, ':')) return (char *)path;

	memcpy(buf, dirActual, dirActualLen);
	strcpy(buf + dirActualLen, path);
...
osd_file *osd_fopen(int pathtype, int pathindex, const char *filename, const char *mode)
{
	...
	logWriteX("(Entrando en osd_fopen /psp/fileio.c) , filename='",filename,"'",sceKernelGetThreadId());
	...
	if( !m_b_initialised )
	{
		...
		{
			openfile[i].handle = INVALID_HANDLE_VALUE;//invalidar todos los handles
		}
		m_b_initialised = 1;
	}
	...
	if (i == MAX_OPEN_FILES)
	{
		logWriteX("(osd_fopen /psp/fileio.c) , filename='",filename,"' , return null por MAX_OPEN_FILES",sceKernelGetThreadId());
		return NULL;
	}
	...
	if (strchr(mode, 'r')){
		disposition = 0, attrib = PSP_O_RDONLY;//O_RDONLY;
		logWriteX("(osd_fopen /psp/fileio.c) , seteo 'r' PSP_O_RDONLY","","",sceKernelGetThreadId());
	}

	if (strchr(mode, 'w')){
		disposition = PSP_O_CREAT | PSP_O_TRUNC, attrib = PSP_O_WRONLY;//O_CREAT | O_TRUNC, attrib = O_WRONLY;
	    logWriteX("(osd_fopen /psp/fileio.c) , seteo 'w' PSP_O_CREAT | PSP_O_TRUNC","","",sceKernelGetThreadId());
	}

	if (strchr(mode, '+')){
		attrib = PSP_O_RDWR;//O_RDWR;
	    logWriteX("(osd_fopen /psp/fileio.c) , seteo '+' PSP_O_RDWR","","",sceKernelGetThreadId());
	}


    logWriteX("(osd_fopen /psp/fileio.c) , compose path es asi: fullpath='","(vacio-destino)","' .",sceKernelGetThreadId());
    logWriteY("(osd_fopen /psp/fileio.c) , pathtype='","val 1","' .",sceKernelGetThreadId(),pathtype,0);
    logWriteY("(osd_fopen /psp/fileio.c) , pathindex='","val 1","' .",sceKernelGetThreadId(),pathindex,0);
    logWriteX("(osd_fopen /psp/fileio.c) , filename='",filename,"' .",sceKernelGetThreadId());

	/* compose the full path */
	compose_path(fullpath, pathtype, pathindex, filename);

   logWriteX("(osd_fopen /psp/fileio.c) , componiendo fullpath='",fullpath,"'",sceKernelGetThreadId());
...
//	file->handle = open( fullpath, attrib | disposition | O_BINARY, S_IRUSR | S_IWUSR  );
    ...
    //strcpy(deathrash,"ms0:/PSP/GAME/mameGberet/");
    //instead of hardcoding it:
    strcpy(deathrash,getCurrentAppDirectory());

    strcat(deathrash,fullpath);

    file->handle = sceIoOpen( /*fullpath*/deathrash, attrib | disposition,0777 );

	logWriteY("(osd_fopen /psp/fileio.c) , <SE INTENTO ABRIR>>>fullpath='",deathrash,"' , val1=fileHandle/fd ",sceKernelGetThreadId(),file->handle,0);
...
//		file->handle = open( fullpath, attrib | disposition | O_BINARY, S_IRUSR | S_IWUSR  );
    ...
    logWriteY("(osd_fopen /psp/fileio.c) v1='file.end, v2=filepos , filename='",filename,"'",sceKernelGetThreadId(),file->end,file->filepos);
...
{
	logWriteY("(osd_ftell) v1='file->offset, ","","",sceKernelGetThreadId(),file->offset,0);
	return file->offset;
}
...
{
strcpy(currentAppDirectory,newCurDir);
}
